...
<!-- 自己css -->
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/back-end/article/css/listallart.css">
</head>

<body>
	<c:import url="/back-end/sidebar.jsp"></c:import>
	
			<div class="workplace h-100 "
				style="top: -1px; margin-bottom: 200px;">
				<h4>文章管理頁面</h4>
				<table id="table_id"
					...
					<thead>
						<tr>
							...
							<th class="showTd">會員編號</th>
							<th class="showTd">文章類別</th>
							<th class="showTd">文章標題</th>
							<th class="showTd">文章狀態</th>
							<th class="showTd">文章瀏覽次數</th>
							<th class="showTd">文章發佈時間</th>
							<th class="showTd">文章最後編輯時間</th>
							<th class="showTd"></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="articleVO" items="${allpost}">
							<tr>
								<td>${articleVO.artId}</td>
								<td>${articleVO.userId}</td>
								<td class="artTypeId">${articleVO.artTypeId}</td>
								<td class="showTd">${articleVO.artTitle}</td>
								<td class="artStatus">${articleVO.artStatus}</td>
								...
								<td>
									<FORM METHOD="post"
										...
										<input type="submit" value="修改狀態"> <input
											type="hidden" name="userId" value="${articleVO.userId}">
										<input type="hidden" name="artId" value="${articleVO.artId}">
										<input type="hidden" name="action" value="AllArtUpdate1">
									</FORM>
								</td>
							</tr>
						</c:forEach>
	...
	<script>
		...
		var artStatus = document.getElementsByClassName("artStatus");
		for (let i = 0; i < artTypeIds.length; i++) {
			if (artStatus[i].innerHTML === "0") {
				artStatus[i].innerHTML = "顯示中";
				artStatus[i].style.color = "#5834f8;";
			} else if (artStatus[i].innerHTML === "1") {
				artStatus[i].innerHTML = "不顯示";
				artStatus[i].style.color = "#6a5655;";
			}
		}
	</script>
